...
const navLinkClass = ({ isActive }: { isActive: boolean }) =>
  `rounded-lg px-4 py-2 text-sm font-semibold transition-colors ${
    isActive ? 'bg-gold text-navy' : 'text-slate-300 hover:bg-white/10 hover:text-white'
  }`
    ...
    <div className="flex min-h-screen flex-col">
      <header className="sticky top-0 z-50 border-b border-white/10 bg-ink/75 backdrop-blur-xl">
        <div className="mx-auto flex h-16 w-full max-w-7xl items-center justify-between gap-4 px-4 sm:px-6">
          ...
          <div className="flex items-center gap-2.5">
            <button
              className="rounded-lg border border-white/25 px-3 py-1.5 text-xs font-bold text-white transition-colors hover:border-gold hover:text-gold"
            ...
            >
              <span className={`h-0.5 w-5 rounded bg-white transition-transform ${menuOpen ? 'translate-y-[7px] rotate-45' : ''}`} />
              <span className={`h-0.5 w-5 rounded bg-white transition-opacity ${menuOpen ? 'opacity-0' : ''}`} />
              <span className={`h-0.5 w-5 rounded bg-white transition-transform ${menuOpen ? '-translate-y-[7px] -rotate-45' : ''}`} />
            </button>
          </div>
        </div>
      ...
      <footer className="border-t border-white/10 bg-black/40">
        <div className="mx-auto flex w-full max-w-7xl flex-col items-center gap-3 px-4 py-9 text-center sm:px-6">
          <Logo size={30} />
          <p className="text-sm text-slate-400">{company.slogan[lang]}</p>
      ...
      <motion.a
        ...
        whileTap={{ scale: 0.95 }}
        className="fixed bottom-5 right-4 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-[#25d366] shadow-lg shadow-[#25d366]/40"
      >
